...
export function VideoInputSection({
  ...
  const handleUrlAdd = async () => {
    ...
    const validUrls = allUrls.filter(url => isValidVideoUrl(url));
    const invalidUrls = allUrls.filter(url => !isValidVideoUrl(url));
    ...
    if (invalidUrls.length > 0) {
      toast({
        variant: "destructive",
        title: "Invalid URLs",
        description: `These URLs are not from supported video platforms: ${invalidUrls.join(", ")}`
      });

      if (validUrls.length === 0) return; // Stop if no valid URLs
    }

    if (validUrls.length > 0) {
      setLoading(true)
      try {
        const newVideos = await Promise.all(validUrls.map(async (url) => {
          try {
